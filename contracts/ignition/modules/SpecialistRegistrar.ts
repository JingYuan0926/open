...
export default buildModule("SpecialistRegistrarModule", (m) => {
  ...
  // Pass via --parameters '{"SpecialistRegistrarModule":{"parentNode":"0x..."}}'.
  const parentNode = m.getParameter<`0x${string}`>("parentNode");

  const registrar = m.contract("SpecialistRegistrar", [
    nameWrapper,
    resolver,
    parentNode,
  ]);
